...
def lagrange_coef(participants, x_i):
    ...
    return lagrange(participants, x_i, 0)
...
class Dealer:
    ...
    def create_signer(self, x):
        ...
        return FrostSigner(x, y, self.public_polynomial)
...
class FrostSigner:
    ...
    def sign(self, context):
        ...
        if not context.verify(
            partial_sig, self.private_nonce_share.public_share, self.x
        ):
...
class FrostCoordinator:
    ...
    def compute_sig(self):
        ...
        for x in self.participants:
            if not self.partial_sigs.get(x):
                raise RuntimeError("Not everyone has registered a nonce")
        ...
        if not self.group_point.verify_schnorr(self.signing_context.msg, signature):
